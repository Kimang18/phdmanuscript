...
\begin{proof}
    ...
    In states $(3,1)$, $\gL$ activates Arm $1$ with probability $p_1$ and in $(3,2)$, $\gL$  rests Arm $1$ with probability $p_2$ where $p_1$ and $p_2$ are our degree of freedoms to minimize the expected regret.

    Facing $M^a$, $\gL$ ends up in $\gX^1$ with probability $(1-p_1)$ when starting in $(3,1)$ and with probability $(1-p_2)$ when starting in $(3,2)$.
    Then, its expected regret is
    \begin{align*}
        \ex{\Reg(\gL,M^a,T)}
        &=0.5\Bigl((1-p_1)\frac{T}2 +p_1\times 0\Bigr) +0.5\Bigl((1-p_2)\frac{T}2 +p_2\times 0\Bigr)\\
        &=\frac{T}2\left(1-\frac{p_1+p_2}2\right).
    \end{align*}
    Facing $M^b$, $\gL$ ends up in $\gX^1$ with probability $p_1$ when starting in $(3,1)$ and with probability $p_2$ when starting in $(3,2)$ and its expected regret is
    \begin{align*}
        \ex{\Reg(\gL,M^b,T)}
        &=0.5\Bigl((1-p_1)\times 0 +p_1\frac{T}2\Bigr) +0.5\Bigl((1-p_2)\times 0 +p_2\frac{T}2\Bigr)\\
        &=\frac{T}2\times\frac{p_1+p_2}2.
    \end{align*}
    Then $\gL$ wants to adjust $p_1$ and $p_2$ such that
    \begin{equation}
        \label{eq:rg_linear}
        \min_{p_1,p_2\in[0,1]}\left(\max\biggl\{\frac{T}2\left(1-\frac{p_1+p_2}2\right), \frac{T}2\times\frac{p_1+p_2}2\biggr\}\right).
    \end{equation}
    For the best possible choice of $(p_1,p_2)$, the minimum value of \eqref{eq:rg_linear} is $\displaystyle\frac{T}4$.
...
This theorem means that a more restrictive assumption on local arm should be defined.
We should mention that the global MDPs used in the proof of Theorem~\ref{thm:non_learnable} are not weakly communicating.
This means that their diameter is infinite.
We will see in the following that there exists restless bandits whose diameter that is exponential in the number of arms.

\subsubsection{Hard to learn restless bandits whose all arms are ergodic}
...
\begin{proof}
    ...
    \begin{figure}[ht]
        ...
        \end{tikzpicture}
        \\
            Arm $1$ & Global MDP having four states.
        \end{tabular}
        \caption{
...
\subsubsection{Diameter, mixing time, and span of the optimal bias}
