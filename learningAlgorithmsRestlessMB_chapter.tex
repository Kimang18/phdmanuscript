...
We consider a discrete-time MDP $M=(\gS,\gA,P,r)$ with a finite state-space $\gS$ and a finite action space $\gA$. If at time $t$, the process is in state $x$ and action $a$ is taken, it earns a reward $r(x,a)$ and jumps to state $y$ with probability $p(y|x,a)$. A stationary Markovian policy for such a MDP is a function $\pi:\gS\to\gA$. A policy $\pi$ induces a Markov chain with transition matrix $P^\pi$ where $P^\pi(x,y) = p(y | x, \pi(x))$. 
...
\begin{itemize}
    \item \emph{recurrent} if for all policy $\pi$, the matrix $P^\pi$ defines a Markov chain where all states are recurrent (equivalently, for all states $x$ and $y$, a Markov chain starting in $x$ will visit $y$ with probability $1$);
    \item \emph{unichain} if for all policy $\pi$, the matrix $P^\pi$ has a single recurrent class plus a possibly empty set of transient states;
...
\end{itemize}
In addition, a recurrent or unichain MDP is called \emph{aperiodic} if all matrices $P^\pi$ are aperiodic.  We say that a MDP is \emph{ergodic} if it is recurrent and aperiodic. 
...
\begin{align*}
    \mathrm{diameter}(M) = \max_{x\neq y}\min_{\pi} \ex{T(y\mid M,\pi,x)}
\end{align*}
...
To be defined?
\cite[Definition 5.1]{wei2020model}
\begin{defn}
    \label{def:mixing_time}
    The mixing time of an ergodic MDP is defined as
    \begin{align*}
        t_{mix} := \max_{\pi}\min\left\{ t\ge1 \mid \lVert (P^\pi)^t(x,\cdot) - \mu^\pi\rVert_1 \le \frac14, \forall x\right\},
    \end{align*}
    that is, the maximum time required for any policy starting at any initial state to make the state distribution $\frac14$-close (in $l_1$ norm) to the stationary distribution.
\end{defn}
...
\begin{thm}
    \begin{enumerate}[label=(\roman*)]
        \item \label{thm:not_ergodic} There exists a Restless bandit whose arms are all recurrent that is noncommunicating. % the term noncommunicating is taken from [Puterman, 1994, page 353]. 
...
\begin{proof}
    \textbf{Proof of \ref{thm:not_ergodic}} -- It is given by the counter-example in which the Restless bandit has two arms as given by \figurename~\ref{fig:recur_non_communicate}.
    For each arm, both "activate" and "rest" induce the same transition.
    In such example, the Restless bandit is a multichain not weakly communicating MDP because if we start at state $(1,a)$ or $(2,b)$, then, the final recurrent class of states is composed of $\{(1,a), (2,b)\}$ and if we start at state $(1,b)$ or $(2,a)$, then, the final recurrent class is $\{(1,b), (2,a)\}$.
    \begin{figure}[ht]
        \centering
        \begin{tabular}{ccc}
        ...
        \end{tabular}
        \caption{A Restless bandit with two 2-state arms that are recurrent.}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[on grid, state/.style={ellipse,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
            ...
            \node[state]  (B) [right = 1.7cm of A]    {$\begin{tabular}{c}26\\35\\36\end{tabular}$};
            \node[state]  (C) [right = 1.7cm of B]    {$\begin{tabular}{c}37\\46\\47\end{tabular}$};
            \node[state]  (D) [right = 1.7cm of C]    {$\begin{tabular}{c}48\\57\\58\end{tabular}$};
            \node[state]  (E) [below = 2.4cm of D]    {$\begin{tabular}{c}51\\68\\61\end{tabular}$};
            \node[state]  (F) [left = 1.7cm of E]    {$\begin{tabular}{c}62\\71\\72\end{tabular}$};
            \node[state]  (G) [left = 1.7cm of F]    {$\begin{tabular}{c}73\\82\\83\end{tabular}$};
            \node[state]  (H) [left = 1.7cm of G]    {$\begin{tabular}{c}84\\13\\14\end{tabular}$};
            \node[text width=2cm] (J) [left = 1.5cm of A] {Recurrent\\Class 1};
            \path[->]
            (A) edge     node{}	(B)
            (B) edge     node{}	(C)
            (C) edge     node{}	(D)
        ...
        \begin{tikzpicture}[on grid, state/.style={ellipse,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
            ...
            \node[state]  (B) [right = 1.7cm of A]    {$\begin{tabular}{c}22\\31\\32\end{tabular}$};
            \node[state]  (C) [right = 1.7cm of B]    {$\begin{tabular}{c}33\\42\\43\end{tabular}$};
            \node[state]  (D) [right = 1.7cm of C]    {$\begin{tabular}{c}44\\53\\54\end{tabular}$};
            \node[state]  (E) [below = 2.4cm of D]    {$\begin{tabular}{c}55\\64\\63\end{tabular}$};
            \node[state]  (F) [left = 1.7cm of E]    {$\begin{tabular}{c}66\\75\\74\end{tabular}$};
            \node[state]  (G) [left = 1.7cm of F]    {$\begin{tabular}{c}77\\86\\84\end{tabular}$};
            \node[state]  (H) [left = 1.7cm of G]    {$\begin{tabular}{c}88\\17\\15\end{tabular}$};
            \node[text width=2cm] (J) [left = 1.5cm of A] {Recurrent\\Class 2};
            \path[->]
            (A) edge     node{}	(B)
            (B) edge     node{}	(C)
            (C) edge     node{}	(D)
        ...
        The first row shows the state transition of the arm: the black arrows represent passive transition and the red ones represent active transition.
...
\begin{thm}
    \label{thm:aperiodic_RB_comm} A Restless bandit whose arms are all ergodic is communicating. 
\end{thm}
\begin{proof}
    to be proved
\end{proof}
...
\begin{thm}
    \label{thm:unichain} If all arms are unichain MDP with at least one state reachable in one step from any other states under both passive and active modes, then, the corresponding RMAB is also a unichain MDP with at least one state reachable in one step from any other global states under any policies.
\end{thm}
\begin{proof}
    For each arm $i\in[n]$, let $z_i$ be the state that is reachable in one step from any other states under both passive and active modes.
    We have that for all $i\in[n],x_i\in\gS_i,a\in\{0,1\}, {P^a_i(x_i, z_i)>0}$.
    For simplification, we write $P^a(x_i,z_i) =P^a_i(x_i, z_i)$.
    For any action $\va\in\bar{\gA}$, any state $\vx,\vy\in\bar{\gS},$ the RMAB's state transition is given by ${\mP^{\va}(\vx,\vy) =\prod_{i=1}^n P^{a_i}(x_i,y_i)}$.
    Then, for any action $\va\in\bar{\gA}$, any state $\vx\in\bar{\gS}$, we have
    \begin{align*}
        \mP^{\va}(\vx,\vz) =\prod_{i=1}^n P^{a_i}(x_i,z_i) > 0,
    \end{align*}
    because $n$ is finite and for any $i$, $P^{a_i}(x_i, z_i)>0$.
...
\section{Diameter and span: counter-examples and positive results}

\paragraph{Diameter can be exponential in the number of arms?}

$\Rightarrow$ Yes, we have an example in which the diameter is exponential in the number of arms.
Consider an example of $n$ arms, each arm is a Markov chain with binary state space and $0.5$ probability of changing state under both actions.
...
\paragraph{Mixing time also?}

$\Rightarrow$ If we define the mixing time as in Definition~\ref{def:mixing_time}, that is, the maximum time required for any policy to make the state distribution $1/4$-close to the stationary distribution, then, the hypothesis of ergodicity on all arms is not enough to define a mixing time for RMAB because a RMAB can be multichain by Theorem~\ref{thm:ergodic_arms_multichain_RB} and the corresponding policy does not have unique stationary distribution.

If RMAB is ergodic, then, the mixing time is defined?
We can construct an ergodic RMAB problem having infinite mixing time.
Let $P^m$ be the transition matrix of the arm given in \figurename~\ref{fig:local_ergodic_multichain_RB}.
Let $U$ be the transition matrix such that the state transition is uniform in any state.
Any arms having $U$ as the transition matrix are ergodic and the RMAB problem that is composed of such arms is ergodic too.
Now, consider the arms having the transition matrix $P=(1-\varepsilon)P^m +\varepsilon U$ for $\varepsilon\in(0,1)$.
Such arms are ergodic and the RMAB problem having such arms is also ergodic.
However, the mixing time of such RMAB problem tends to infinity when $\varepsilon$ tends to $0$ because when $\varepsilon=0$ the problem is multichain by Theorem~\ref{thm:ergodic_arms_multichain_RB}.
Hence, the hypothesis of ergodicity on RMAB problem is still not enough for mixing time to be defined.
%\KK{Is the conclusion of this paragraph correct?}

In this case, the mixing time is not bounded.

\paragraph{What about span?}
...
\KK{Add text}
After $T$ time steps, the regret of learner is given by
\begin{align}
