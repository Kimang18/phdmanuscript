...
We discuss the existing works in reinforcement learning (RL) with restless bandit problems in Section~\ref{ch:restless:related}.
...
We show that:
\begin{itemize}
    \item no matter how good a learning algorithm is and how many total steps it is allowed to learn, there is a restless bandit, all of whose arms are unichain, that makes the learner suffer an expected regret that is linear in total steps
...
The work of \cite{akbarzadeh2022learning} adapts TSDE \cite{ouyang2017learning} to the fully observed restless bandit problem and ``successfully'' removes the exponentiality in the number of arms from the state size of the bandit in the Bayesian regret bound of RB-TSDE, the modified version of TSDE.
...
With Theorem~\ref{thm:ergodicity_coeff}, Assumption~\ref{asmp:ergodic} implies that there exists $\varepsilon>0$ such that any bandit $M'\in\mathrm{supp}(\phi)$ satisfies the condition \eqref{eq:gamma_ep} of Theorem~\ref{thm:ergodicity_coeff}, and $\beta^*=1-\varepsilon^n$.
%where $\varepsilon>0$ is usually very small.
...
In fact, it is impossible to do so for general restless bandits as we have seen in this chapter that the MDP parameters of some bandits such as the global diameter, or the span of global bias function can be exponential in the number of arms.
