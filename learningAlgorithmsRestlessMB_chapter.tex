...
We then recall the notations and the problem formulation of the average reward restless bandit in Section~\ref{ch:restless:sec:restless}.
...
\section{Restless bandit with average reward criterion}
...
Each arm $\langle\gS_i, \{0,1\}, \{\vr^0_i, \vr^1_i\}, \{\mP^0_i, \mP^1_i\}\rangle$ is an MDP with a finite state space $\gS_i$ of size $S$ and a binary action space $\{0,1\}$, where $0$ denotes the action ``rest'', and $1$ denotes the action ``activate''.
If arm $i$ is in state $s_i$, and the decision maker executes $a_i\in\{0,1\}$, the arm incurs a random reward with the expected value $r^{a_i}_i(s_i)$ and transitions to state $s'_i\in\gS_i$ with a probability $P_i^{a_i}(s_i, s'_i)$.
...
So, we will drop the index $i$ from the expected reward and transition if no confusion is possible: we denote them by $r^{a_i}(s_i)$ instead of $r^{a_i}_i(s_i)$ and by $P^{a_i}(s_i, s'_i)$ instead of $P_i^{a_i}(s_i, s'_i)$.
...
Without loss of generality, we assume that for any global state $\vs\in\gX$ and global action $\va\in\gA(m)$, the expected reward is bounded like $\sum_{i=1}^{n}r^{a_i}(s_i)\in[0,r_{max}]$.
...
However, it is shown in \cite[Theorem~4]{papadimitriou1994complexity} that computing an optimal policy in  restless bandit with average reward is PSPACE-hard.
...
\begin{thm}[Non-learnable restless bandit]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item \label{it:non_learnable2} For any learning algorithm $\gL$, any total steps $T\ge2$, there is a weakly communicating restless bandit whose arms are unichain and have a bounded span of local bias function under any policy such that the expected regret of $\gL$ is at least $\frac{T}5$.
    \end{enumerate}
\end{thm}
\begin{proof}
...
\begin{proof}
    ...
    We have that $P^{a_i}(s_i, z_i)>0$ for all $i\in[n], x_i\in\gS_i,a_i\in\{0,1\}$.
    For any global action $\va\in\gA(m)$, any global states $\vs,\vs'\in\gX,$ the bandit $M$'s state transition is given by ${p(\vs' \mid \vs, \va) =\prod_{i=1}^n P^{a_i}(s_i, s'_i)}$.
    Then, for any action $\va\in\gA(m)$, any state $\vs\in\gX$, we have
    \begin{align*}
        p(\vz \mid \vs, \va) =\prod_{i=1}^n P^{a_i}(s_i,z_i) > 0,
    \end{align*}
    because $n$ is finite and for any $i$, $P^{a_i}(s_i,z_i)>0$.
...
\end{equation}
RB-TSDE \cite{akbarzadeh2022learning} exploits the structure of restless bandit by choosing a prior distribution $\phi_i$ for each unknown arm $\langle\gS_i, \{0,1\}, \{\vr^0_i,\vr^1_i\}, \{\mP^0_i,\mP^1_i\}\rangle$ before the learning.
At time step $t^k$, RB-TSDE uses the collected observations $o_{t^k}$ to derive a posterior $\phi_i(\cdot\mid o_{t^k})$, and draws a sample $(\{\vr^{0k}_i,\vr^{1k}_i\},\{\mP^{0k}_i,\mP^{1k}_i\})$ according to $\phi_i(\cdot\mid o_{t^k})$ for each arm $i$.
The samples $\{(\{\vr^{0k}_i,\vr^{1k}_i\},\{\mP^{0k}_i,\mP^{1k}_i\})\}_{i\in[n]}$ is then used to compute Whittle index policy $\pi^k$.
...
\begin{asmp}
    ...
    For any $i\in[n]$ and $(\{\vr^{0'}_i,\vr^{1'}_i\}, \{\mP^{0'}_i,\mP^{1'}_i\})\in\mathrm{supp}(\phi_i)$, the arm $\langle\gS_i,\{0,1\},\{\vr^{0'}_i,\vr^{1'}_i\}, \{\mP^{0'}_i,\mP^{1'}_i\}\rangle$ is indexable.
\end{asmp}
\begin{asmp}
    \label{asmp:unichain}
...
\begin{align}
    ...
    &= \sum_{k=1}^{K_T}\sum_{t=t^k}^{t^{k+1}-1} \Bigl(g^\widx_M -\sum_{i=1}^{n}r^{a_{t,i}}(s_{t,i})\Bigr) \label{ch:restless:eq:regret1}\\
    &\qquad +\underbrace{\sum_{i=1}^{n}\Bigl(r^{a_{t,i}}(s_{t,i})-r_{t,i}\Bigr)}_{\Delta_{t,0}} \nonumber
\end{align}
...
\begin{equation}
    ...
    0 =  \sum_{i=1}^nr^{a_{t,i}k}(s_{t,i}) -g^{\pi^k}_{M^k} +\sum_{\vs'\in\gX}p^k(\vs' \mid \vs_t, \va_t)h^{\pi^k}_{M^k}(\vs') -h^{\pi^k}_{M^k}(\vs_t).
\end{equation}
...
\begin{align*}
    \Reg(\textnormal{RB-TSDE},M,T)
    &=\sum_{k=1}^{K_T}\sum_{t=t^k}^{t^{k+1}-1} \underbrace{(g^\widx_M{-}g^{\pi^k}_{M^k})}_{\Delta_{t,1}} {+}\underbrace{\sum_{i=1}^n\Bigl(r^{a_{t,i}k}(s_{t,i}){-}r^{a_{t,i}}(s_{t,i})\Bigr)}_{\Delta_{t,2}} \\
...
\begin{align*}
    ...
    \le \sum_{i=1}^{n} \sum_{s'_i\in\gS_i} \abs{P^{a_ik}(s_i, s'_i) -P^{a_i}(s_i, s'_i)},
\end{align*}
where $\sum_{s'_i\in\gS_i} \abs{P^{a_ik}(s_i, s'_i) -P^{a_i}(s_i, s'_i)}$ is the concentration of the local transition, which is bounded proportionally to $\sqrt{S}$ using Weissman's inequality.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \sum_{\vz\in\gX} \min\left\{\prod_{i=1}^nP^{a_i}(s_i, z_i), \prod_{i=1}^nP^{a'_i}(s'_i, z_i)\right\} \\
        &\ge \sum_{\vz\in\gX}\prod_{i=1}^n \min\Big\{P^{a_i}(s_i, z_i), P^{a'_i}(s'_i, z_i)\Big\} \\
        &= \prod_{i=1}^n \left(\sum_{z_i\in\gS_i} \min\Big\{P^{a_i}(s_i, z_i), P^{a'_i}(s'_i, z_i)\Big\}\right).
    \end{align*}
    Since for any $i\in[n]$, $\gamma_i<1$, we have that
    \begin{align*}
        \sum_{z_i\in\gS_i} \min\{P^{a_i}(s_i, z_i), P^{a'_i}(s'_i, z_i)\}
        &\ge \min_{\substack{x_i,y_i\in\gS_i \\a,a'\in\{0,1\}}} \sum_{z_i\in\gS_i} \min\{P^{a}(s_i, z_i), P^{a'}(s'_i, z_i)\} \\
        &= 1-\gamma_i >0.
    \end{align*}
    ...
    \begin{align*}
        \prod_{i=1}^n \left(\sum_{z_i\in\gS_i} \min\{P^{a_i}(s_i, z_i), P^{a'_i}(s'_i, z_i)\}\right)
        &\ge \varepsilon^n.
    \end{align*}
