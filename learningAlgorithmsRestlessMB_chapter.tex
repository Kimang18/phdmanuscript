...
\begin{proof}
    ...
    An optimal polic is the following $\pi^*(1,1)=1, \pi^*(1,2)=2, \pi^*(2,1)=1, \pi^*(2,2)=2$.
    ...
    The optimal gain is $g^*=1$ and the span of the optimal bias is $sp(\vh^*)=\frac1\delta$.
    ...
    The regret gap of suboptimal policies when staying in $\gX^2$ is then $g^*-1/2=1/2$.
    ...
    This means that $\tilde{\gL}$ spends, in expectation, $\landauOmega(4/\delta)$ time steps in $\gX^2$ before leaving to $\gX^1$ and get the maximum gain.
    Then, the regret of $\tilde{\gL}$ is $\landauOmega(2/\delta)$.
...
\end{proof}
    ...
    \begin{figure}[htbp]
        ...
        \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
            ...
            \node[state, black!45!green]  (B) [below = 2cm of A]   {$2$};
            \path[->]
            (A) edge[bend left=75]     node{$0,p{=}\varepsilon$}	(B)
            (A) edge[loop above]     node[above]{$0,p{=}1{-}\varepsilon$}	(A)
            (A) edge[bend left, dashed, red]     node{$0$}	(B)
    	    (B) edge[bend left=75]     node{$1$}	(A)
            (B) edge[bend left, dashed, red]     node{$0$}	(A);
        \end{tikzpicture}
        ...
        \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
                ...
                \path[->]
                (A) edge[bend right=80, black!30!purple]     node[above]{$0,p{=}1$}	(D)
                (B) edge[bend left, black!45!green]     node{$0,p{=}1{-}\varepsilon$}	(A)
                (B) edge[bend right, black!45!green]     node[below]{$0,p{=}\varepsilon$}	(C)
                (C) edge[bend right=80, black!30!purple]     node[above]{$0,p{=}1$}	(B);
                % transition of optimal actions
                \draw[-{Stealth}{Stealth}]
                (A) edge[bend right, black!45!green]     node[above]{$0,p{=}\varepsilon$}	(D)
                (A) edge[bend left, black!45!green]     node{$0,p{=}1{-}\varepsilon$}	(B)
                (B) edge[bend right=80, black!30!purple]     node[below]{$1,p{=}1$}	(C)
                (C) edge[bend right, black!45!green]     node[above]{$1,p{=}1$}	(B)
                (D) edge[bend right, black!45!green] node[below]{$1,p{=}1$} (A)
                (D) edge[bend right=80, black!30!purple] node[below]{$1,p{=}1$} (A);
