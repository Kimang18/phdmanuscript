...
\KK{Add in the intro somewhere that restless span in the regret differently from rested}
...
For partially observed restless bandit, the work of \cite{ortner2012regret} derives colored-UCRL algorithm, a modified version of UCRL2, that achieves a regret sublinear in total steps.
...
In addition, we say that a MDP is \emph{recurrent} if for all policy $\pi$, the transition matrix $\mP^\pi$ defines a Markov chain where all states are recurrent (equivalently, for all states $s$ and $s'$, a Markov chain starting in $s$ will visit $s'$ with probability $1$).
...
In each arm, the state transitions of action activate are drawn in back color and those of action rest are drawn in dashed red color.
...
So, it should be safe to say that ``if the unknown parameters of the MDP are well estimated, then the regret is bounded sublinearly in the total steps $T$''.
...
Then, the expected regret in learning such a restless bandit should be bounded not only linearly in $n$ but also sublinearly in the total steps $T$.
...
Yet, the reason that \ref{it:non_learnable2} is proposed is because the global MDPs in \ref{thm:non_learnable1} are not weakly communicating (see \figurename~\ref{ch:mdp:fig:mdp_class} for the MDP classification).
...
\subsubsection{Restless bandits whose arms all have no local transient state}
...
\begin{thm}[Multichain restless bandit]
    ...
    \begin{enumerate}[label=(\roman*)]
        \item[]
        \item \label{thm:not_ergodic} Among restless bandits whose arms all  are recurrent, there is a bandit that is not weakly communicating. % the term noncommunicating is taken from [Puterman, 1994, page 353]. 
...
\begin{proof}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[on grid, state/.style={ellipse,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
            ...
            \end{tikzpicture} \\
            Arm $1$ & Arm $2$ \\
            \multicolumn{2}{c}{Each transition happens with probability $0.5$.}
        \end{tabular}
        \caption{
...
\begin{thm}
    ...
    \begin{enumerate}[label=(\roman*)]
        \item[]
        \item \label{thm:diam} There is a restless bandit having $n$ 2-state arms, all arms have a diameter of size $2$, that is ergodic and has a diameter of size $2^n$. % the term noncommunicating is taken from [Puterman, 1994, page 353]. 
...
\begin{proof}
    ...
    Now, consider two 8-state restless arms having transition functions $u_1$ and $u_2$ where $u_1$ and $u_2$ induce the same state transition: any local state $s\in[8]$ transitions to any other lcaol state $s'\in[8]$ with probability $\frac18$ under either action rest or activate.
...
This term does not imply neither the learning algorithm has a regret bound sublinear in the total steps nor its regret bound is explicitly linear in the number of arms.
...
For infinite-horizon RL problems, two of the most popular general-purpose RL algorithms are UCRL2 \cite{jaksch2010near} from OFU method and TSDE \cite{ouyang2017learning} from Bayesian method.
Similarly to we have seen in Chapter~\ref{ch:learning_rested}, the regret bound of both algorithms when directly applied to restless bandit $M$ described in Section~\ref{ch:restless:sec:restless} is $\landauO\Bigl(DS^n\sqrt{{n \choose m}T}\Bigr)$ where $D$ is the global diameter of the bandit $M$. This regret bound, which is exponential in the number of arms $n$, follows from Table~\ref{ch:rl:tab:infinite} and the fact the state and action sizes of $M$ are $S^n$ and ${n \choose m}$ respectively.
There are two issues in this regret bound.
One noticeable problem is the term $S^n$ that is the state size of $M$.
And the other implicit problem is te global diameter of $M$. 
As we have seen in Theorem~\ref{thm:parameters}, it is perfectly possible that there is a bandit with a diameter that exponential in the number of arms.
%TODO
...
For optimism, we have shown in Chapter~\ref{ch:learning_rested} that optimistic algorithms that apply confidence bonus on arm's transition cannot leverage index policy to choose an imagined MDP $M^k$ with index policy $\pi^k$ that guarantees the optimism.
...
As we have seen in this chapter, the parameters of the bandit such as the diameter $D$, the mixing time, or the upper bound on span $H$ can be exponential in the number of arms.
...
\KK{Include back the following}

Last but not least, the following theorem shows how the assumption on the ergodicity coefficient of local arms affects the ergodicity coefficient of the bandit.
\begin{thm}
