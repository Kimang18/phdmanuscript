...
\KK{How do we define bias? What MDP structure should we assume?}
...
This limit always exists for finite-state process (see Section~A.4 of Appendix~A of \cite{puterman2014markov}).
...
Let $\mP^m$ be the transition matrix of the arm given in \figurename~\ref{fig:local_ergodic_multichain_RB}.
Let $\mU$ be the transition matrix such that the state transition is uniform in any state.
Any arm having $\mU$ as the transition matrix is ergodic and the RMAB problem that is composed of such arms is ergodic too.
