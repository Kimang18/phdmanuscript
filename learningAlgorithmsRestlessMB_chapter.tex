...
\begin{equation}
    ...
    g^\pi(s):=\lim_{T\to+\infty}\frac1T \E^\pi\left[ \sum_{t=1}^{T} \sum_{i=1}^nr_{t,i} \mid \vs_1=\vs\right].
\end{equation}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
        ...
        \node[state, black!45!green]  (C) [below left = 1cm and 1.5cm of A]   {$3$};
        \path[->]
        (A) edge[bend left=60]     node{}	(B)
        (A) edge[bend left, dashed, red]     node{}	(B)
	    (B) edge[bend left=60]     node{}	(A)
	    (C) edge[bend left]     node{}	(A)
        (C) edge[bend right, dashed, red]     node{}	(B)
	    (B) edge[bend left, dashed, red]     node{}	(A);
    \end{tikzpicture}
    &
    \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
    ...
    \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
        ...
        \path[->]
        (A) edge[bend left, black!45!green]     node{}	(B)
        (A) edge[bend left=60, black!30!purple]     node{}	(B)
	    (B) edge[bend left, black!45!green]     node{}	(A)
	    (B) edge[bend left=60, black!30!purple]     node{}	(A)
	    (C) edge[bend left, black!45!green]     node{}	(D)
	    (C) edge[bend left=60, black!30!purple]     node{}	(D)
        (D) edge[bend left, black!45!green] node{} (C)
        (D) edge[bend left=60, black!30!purple] node{} (C)
        (E) edge[ black!30!purple] node{} (B)
        (E) edge[black!45!green] node{} (C)
        (F) edge[ black!30!purple] node{} (D)
        (F) edge[black!45!green] node{} (A);
    \end{tikzpicture}
    \\
        Arm $1$ & Arm $2$ & Global MDP
    \end{tabular}
    \caption{A restless bandit with two arms. TODO: Add more explanation}
    \label{fig:hard}
\end{figure}
...
\begin{thm}
    There exists an ergodic RMAB in which any (near-)optimal learning algorithm suffers a regret that is upper bounded linearly in $T$, the total number of time steps.
\end{thm}
\begin{proof}
    We show this theorem by constructing a RMAB whose span of the optimal bias equals $T$.
    ...
    This RMAB is ergodic.
    The reward in local states $1$ and $a$ is $1$, and states $2$ and $b$ is $0$.
    To maximize the average reward in this RMAB, we need Arm~$1$ to stay in state $1$ as frequently as possible.
    ...
    Using Bellman evaluation equations for $\pi^*$ with the condition that $h^*(2b)=0$, we get
    \begin{align*}
    ...
    Following \cite{jaksch2010near, zhang2019regret}, the minimax lower bound in general MDPs is $\landauOmega(\sqrt{sp(\vh^*)SAT})$ and it is unimprovable.
    This means that any (near-)optimal learning algorithm suffers a regret bounded by $\tilde{\landauO}(\sqrt{sp(\vh^*)SAT})$ in any communicating MDP.
    In the RMAB above, which is an ergodic MDP, this upper bound becomes $\tilde{\landauO}\Bigl(\sqrt{SAT(1+\varepsilon)/(2\varepsilon)}\Bigr)$.
    We conclude the proof by letting $\varepsilon=1/(2T-1)$ which results in $(1+\varepsilon)/(2\varepsilon)=T$.
    %$\displaystyle\Omega\left(\sqrt{\frac{1+\varepsilon}{2\varepsilon}T}\right)$.
    ...
    \KK{Define $H=sp(\vh^*)$ requires that the MDP is weakly communicating because we need the optimal gain to be state independent. This is what is used in \cite{bartlett2012regal,zhang2019regret}}
    \begin{figure}[ht]
        \centering
        \begin{tabular}{ccc}
        \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
            ...
            \node[state]  (B) [below = 2cm of A]   {$2$};
            \path[->]
            (A) edge[bend left, color=red, dashed]     node{}	(B)
            (A) edge[bend left=80]     node{$1-\varepsilon$}	(B)
            (A) edge[loop above]     node{$\varepsilon$}	(A)
    	    (B) edge[bend left, color=red, dashed]     node{}	(A)
