...
We show in the following theorem that the assumption that each arm is unichain and has a bounded local span of bias under any policy is not enough to make a restless bandit learnable.
...
\begin{thm}[Non learnable restless bandit]
    ...
    \begin{enumerate}[label=(\roman*)]
        \item \label{it:non_learnable1} For any learning algorithm $\gL$, there is a restless bandit having arms all unichain, each arm has a bounded local span of bias under any local policy, such that for any total number of time steps $T\ge2$, the expected regret of $\gL$ at least $\frac{T}4$.
        \item \label{it:non_learnable2} For any learning algorithm $\gL$, any total number of time steps $T\ge2$, there is a restless bandit having arms all unichain, each arm has a bounded local span of bias under any local policy, that is weakly communicating and such that, the expected regret of $\gL$ at least $\frac{T}5$.
    \end{enumerate}
\end{thm}
\begin{proof}
...
\begin{thm}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item \label{thm:span} there exists a bandit, whose arms all have a bounded local span of bias under any local policy, that is communicating and has a global span of optimal bias in the same order of $\landauO(1/\varepsilon)$ for any $\varepsilon\in(0,1)$.
    \end{enumerate}
\end{thm}
\begin{proof}
...
\begin{align}
    \Reg(\gL,M,T) := Tg^*_M - \sum_{t=1}^{T} \sum_{i=1}^nr_{i,t}. \label{eq:regret}
\end{align}
...
We recall from \eqref{eq:bayes_rg} the Bayesian regret of $\gL$.
\begin{align}
    \BayReg(\gL, \phi, T) := \ex{\Reg(\gL, M, T)}.
\end{align}
...
We require $\pi^k$ to induce state-independent gain in order to have the Bellman evaluation equation: for global state $\vs$, $\va=\pi^(\vs)$,
\begin{align*}
    g^{\pi^k}_{M^k} +h^{\pi^k}_{M^k}(\vs) = \sum_{i=1}^nr^k(s_i, a_i) +\sum_{\vs'\in\gX}p^k(\vs' \mid \vs, \va)h^{\pi^k}_{M^k}(\vs').
\end{align*}
Then,
\begin{align*}
    \Reg(\gL, M, T)
    &=\sum_{k=1}^{K_T}\sum_{t=t^k}^{t^{k+1}-1} \underbrace{(g^*_M{-}g^{\pi^k}_{M^k})}_{\Delta_1} {+}\underbrace{\sum_{i=1}^n(r^k{-}r)(s_{t,i},a_{t,i})}_{\Delta_2} \\
    &\quad {+}\underbrace{\sum_{\vs'\in\gX}(p^k-p)(\vs' \mid \vs_t, \va_t)h^{\pi^k}_{M^k}(\vs')}_{\Delta_3} {+}\underbrace{h^{\pi^k}_{M^k}(\vs_{t+1}){-}h^{\pi^k}_{M^k}(\vs_{t})}_{\Delta_4} \\
    &\quad {+}\underbrace{\sum_{\vs'\in\gX} p(\vs' \mid \vs_t, \va_t)h^{\pi^k}_{M^k}(\vs')-h^{\pi^k}_{M^k}(\vs_{t+1})}_{\Delta_5}.
\end{align*}
The five quantities are bounded as the following.
\begin{itemize}
    \item The second term $\Delta_2$ is bounded by sub-Gaussian inequality (or simply zero if the rewards are deterministic);
    \item The first term $\Delta_1$ and the last term $\Delta_5$ are bounded based on the approach used: for optimism, $\Delta_1$ is non-positive due to optimism and the sum of $\Delta_5$ is bounded by Azuma-Hoeffding inequality for martingale difference sequence. For Bayesian methods, the expected value of $\Delta_1$ is bounded by $K_T$ and of $\Delta_5$ is zero
    \item the sum of $\Delta_4$ is a telescopic sum which is bounded by $K_TH$ where $H:=\max\{\max_{\pi}sp(\vh^\pi_M)\}$.
\end{itemize}
