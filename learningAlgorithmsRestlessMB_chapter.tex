...
Such a bandit is an MDP that suffers from the curse of dimensionality and general-purpose learning algorithms are not efficient when directly applied. Recently, a few learning algorithms have been specifically designed to work for restless bandits. Yet, they most often work only for very particular subclasses of restless bandits, or they hold under conditions that are very hard to verify.
Hence, in this chapter, we provide some arguments to explain why those conditions are needed when learning in restless bandits. We show that the properties of the local arms (like ergodic or small diameter) do not, in general, imply similar properties for the bandit. This implies that defining a class of restless bandit that has desirable properties (like small diameter) is difficult. 
Finally, we discuss a few issues when learning in general class of restless bandits and present RB-TSDE \cite{akbarzadeh2022learning} and its regret analysis under a few technical assumptions.

In Section~\ref{ch:restless:contribute}, we provide the context of learning in restless bandit problems and present our contribution in this chapter.
We discuss the existing works in reinforcement learning (RL) with generic MDPs and with restless bandit problems in Section~\ref{ch:restless:related}
...
In Section~\ref{ch:restless:sec:generic}, we discuss the requirement when using Whittle index policy as the baseline policy in regret definition.
...
\end{itemize}
So, the regret bound of those general-purpose algorithms is not scalable with the number of arms.
...
Yet, no clear dependency between the diameter, the mixing time, or the upper bound on span of the optimal bias that appear in their regret bound and number of arms are given.
...
In this chapter, we provide a few results that support this argument and study how the assumption on arms' structure translates in the structure of the bandit.
We show that:
\begin{itemize}
...
All of our results suggest that the MDP parameters of restless bandit can be exponential in the number of arms in general class of restless bandit.
This calls for work to design a subclass of restless bandits whose MDP parameters can expressed polynomially in the number of arms.
...
the first direction consists in learning the Whittle index of restless Markovian arms in an unknown fully observed restless bandit.
...
Yet both works \cite{gibson2021novel, fu2019towards} only provide the numerical proof of convergence to the correct Whittle index and no theorectical proof is given.
Using the ideas from two-time-scale stochastic approximation (see \eg, \cite{abounadi2001learning, schwartz1993reinforcement}), the work of \cite{avrachenkov2022whittle} proposes a two-time-scale QL algorithm for learning the Whittle index of any unknown indexable restless arms. Their algorithm maintains a Q-function and a vector of penalty separately, updates Q-function using QL algorithm and the vector of penalty using stochastic iterative algorithm with a slower time scale update compared to the QL algorithm.
...
The second direction is to design a model-based learning algorithm with theorectical performance guarantee.
...
The same discussion goes to the work of \cite{jung2019thompson} that adapts TSDE to the setting of restless bandit and assumes a condition similar to \cite{ortner2012regret}.
...
Yet, we must mention that their regret bound is proportional to $T^{2/3}$ and their result is valid for a very restrictive class of restless bandits in which each arm is a birth-death Markov reward process.
...
In the following, we study the structure of the bandit when all of the local arms are well-structured.
...
Yet, the reason that \ref{it:non_learnable2} is proposed is that the bandits in \ref{thm:non_learnable1} are not weakly communicating (see \figurename~\ref{ch:mdp:fig:mdp_class} for the MDP classification).
...
\begin{proof}
    ...
    In such example, the restless bandit is a multichain and not weakly communicating MDP because there are two recurrent classes under any policy: one composed of $\{(1,1), (2,2)\}$ and the other one is $\{(1,2), (2,1)\}$.
    \begin{figure}[ht]
        \centering
        \begin{tabular}{ccc}
        \begin{tikzpicture}[on grid, state/.style={circle,draw}, >= stealth', auto, prob/.style = {inner sep=1pt,font=\scriptsize}]
...
Finally, we provide a few results that show that the parameters of the bandit such as the diameter, the mixing time, and span of bias of any global policy can be arbitrarily large.
\begin{thm}
    \label{thm:parameters}
    \begin{enumerate}[label=(\roman*)]
        \item[]
        \item \label{thm:diam} There is a restless bandit having $n$ 2-state arms, all of whose arms have a diameter of size $2$, that is ergodic and has a diameter of size $2^n$. % the term noncommunicating is taken from [Puterman, 1994, page 353]. 
...
\begin{proof}
    ...
    As we mentioned above, for each arm, the span of local bias under any policy is bounded in $[0,1]$ but the bandit $M$ is multichain.
...
Finally, the following theorem shows how the assumption on the ergodicity coefficient of local arms affects the ergodicity coefficient of the bandit.

\begin{thm}[Local and global ergodicity coefficients]
    Consider bandit $M$ described in Section~\ref{ch:restless:sec:restless}.
    ...
    \end{align*}
    Similarly, the ergodicity coefficient $\beta_M$ of the bandit $M$ is defined by
    \begin{align}
        \label{eq:ergodic_M}
        \beta_M = 1-\min_{\substack{\vs,\vs'\in\gX \\\va,\va'\in\gA(m)}} \sum_{\vz\in\gX} \min\{p(\vz \mid \vs, \va), p(\vz \mid \vs', \va')\}.
    \end{align}
...
\end{proof}
We will come back to this result when we discuss the regret analysis of RB-TSDE \cite{akbarzadeh2022learning} in the sections below.
...
\subsection{Discussion on using Whittle index policy as a baseline in regret definition}
...
For the optimism, we have shown in Chapter~\ref{ch:learning_rested} that optimistic algorithms that apply confidence bonus on arm's transition cannot leverage index policy to choose an imagined bandit $M^k$ with index policy $\pi^k$ that guarantees the optimism at episode $k$.
...
In the following, we provide a sketch of proof of the regret bound of RB-TSDE \cite{akbarzadeh2022learning}, that is modified version of TSDE to restless Markovian bandit and a few comments on their result.
...
In this section, we start by presenting how RB-TSDE works and the assumptions needed in \cite{akbarzadeh2022learning}. Next, we provide an overview of regret analysis of RB-TSDE under the assumptions. We finish the section with a discussion on their result.

RB-TSDE is a Bayesian learning algorithm for restless Markovian bandits with average reward criterion.
Extended from TSDE, RB-TSDE updates its policy episodically as the following.
...
\end{equation}
RB-TSDE exploits the structure of restless bandit by choosing a prior distribution $\phi_i$ for each unknown arm $\langle\gS_i, \{0,1\}, r_i, p_i\rangle$ before the learning.
...
The samples $\{(r^k_i,p^k_i)\}_{i\in[n]}$ is then used to compute the Whittle index policy $\pi^k$.

RB-TSDE computes Whittle index policy $\pi^k$ of the sampled bandit $M^k$ because Whittle index policy $\pi^*$ of the unknown bandit $M$ is used as the baseline policy for evaluating the regret of RB-TSDE.
So, the work of \cite{akbarzadeh2022learning} assume three conditions: Let $\mathrm{supp}(\phi_i)$ be the support of prior distribution $\phi_i$ for each arm $i$.
...
By Definition~\ref{ch:rl:defn:rg_infinite}, the regret of RB-TSDE after $T$ steps is given by
\begin{align}
    \Reg(\textnormal{RB-TSDE},M,T) := Tg^*_M - \sum_{t=1}^{T} \sum_{i=1}^nr_{t,i}. \label{ch:restless:eq:regret}
\end{align}
...
\begin{align}
    ...
    &= \sum_{k=1}^{K_T}\sum_{t=t^k}^{t^{k+1}-1} \Bigl(g^*_M -\sum_{i=1}^{n}r(s_{t,i},a_{t,i})\Bigr) \label{ch:restless:eq:regret1}\\
...
\end{equation}
Let $K_T$ be the total number of episodes (or policy updates) up to time $T$.
Then, adding \Eqref{eq:be_imagined} and $0=h^{\pi^k}_{M^k}(\vs_{t+1})-h^{\pi^k}_{M^k}(\vs_{t+1})$ to \Eqref{ch:restless:eq:regret1}, and regrouping terms give
\begin{align*}
    \Reg(\textnormal{RB-TSDE},M,T)
    &=\sum_{k=1}^{K_T}\sum_{t=t^k}^{t^{k+1}-1} \underbrace{(g^*_M{-}g^{\pi^k}_{M^k})}_{\Delta_{t,1}} {+}\underbrace{\sum_{i=1}^n\Bigl(r^k(s_{t,i},a_{t,i}){-}r(s_{t,i},a_{t,i})\Bigr)}_{\Delta_{t,2}} \\
...
\cite{akbarzadeh2022learning} exploit this structure as the following:
...
In summary, RB-TSDE enjoys the following Bayesian regret bound.
...
Under Assumptions~\ref{asmp:indexable}-\ref{asmp:ergodic}, 
    ...
    \end{equation}
\end{prop}
This result presents an exponential improvement compared to the current best general-purpose RL algorithms whose regret bound is $\tilde{\landauO}\Bigl(\sqrt{HS^n{n \choose m}T}\Bigr)$.

However, the upper bound on the span of global bias function $H=\displaystyle\frac{2r_{max}}{1-\beta^*}$ has no explicit dependency with the number of arms.
In fact, if any bandit $M'\in\mathrm{supp}(\phi)$ satisfies the condition \eqref{eq:gamma_ep} of our Theorem~\ref{thm:ergodicity_coeff}, then $\beta^*=1-\varepsilon^n$.
This implies that $H=\displaystyle\frac{2r_{max}}{\varepsilon^n}$ which is exponential in $n$.
...
We believe that it is challenging to derive the upper bound on the span of global bias function $H$ or the global diameter $D$ of $M$ that are explicitly polynomially in $n$.
In fact, it is impossible to do so for general restless bandits as we have seen in this chapter that the parameters of some bandits such as the diameter, the mixing time, or the span of global bias function can be exponential in the number of arms.
This calls for restrictive assumptions on bandits in order to to design an algorithm with a regret bound explicitly polynomially in the number of arms.
This also calls for algorithms that directly learns Whittle index of the unknown bandit in the direction of \cite{fu2019towards, gibson2021novel, nakhleh2021neurwin, avrachenkov2022whittle}.
