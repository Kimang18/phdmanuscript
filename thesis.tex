...
\iftotalcompilation
\part{Background on Markov decision process, reinforcement learning, and Markovian bandits}
...
In Chapter~\ref{ch:rl}, we recapitulate the basics and several existing results in reinforcement learing literature.
In Chapter~\ref{ch:mb}, we present the formalism of Markovian bandits and existing theoretical results.
...
In this part, we present our results on indexability and index computation for Markovian bandits.
In Chapter~\ref{ch:indexability}, we discuss about the ambiguities in the classical definition of indexability and propose a new definition that is univocal for any restless bandits.
In Chapter~\ref{ch:index_computation}, we develop a unified algorithm that computes indexability and the Whittle index in either discounted or undiscounted restless bandits as well as the Gittins index in discounted rested bandits.
...
We also introduce new notations when developping our algorithm.
...
In this part, we focus on learning problem in Markovian bandits.
We start by considering the learning with discounted rested bandits in Chapter~\ref{ch:learning_rested}.
An efficient policy for discounted rested bandits is the Gittins index policy.
We show that MB-PSRL, a modified PSRL for rested bandits, can leverage Gittins index to have a regret bound and runtime scalable in the number of arms.
