...
Following Whittle's development \cite{whittle1988restless}, the constraint
\begin{align*}
    \text{for any time }t\ge1, \sum_{i=1}^{n}a_{t,i}=m \text{ is relaxed to } \lim_{T\to+\infty}\frac1T \E^\pi\left[\sum_{i=1}^n \sum_{t=1}^{T} a_{t,i} \mid \vs_1=\vs\right]=m.
\end{align*}
...
From \cite{whittle1988restless}, if arm $i$ is indexable, then the Whittle index of state $s_i$ is the critical value $\widx(s_i)$ such that action rest and activate are equivalent in \eqref{ch:mb:eq:bellman}:
\begin{align}
    r(s_i,0) +\sum_{s'_i\in\gS_i}p(s'_i\mid s_i,0)h^*(s'_i)
    =r(s_i,1)-\lambda(s_i) +\sum_{s'_i\in\gS_i}p(s'_i\mid s_i,1)h^*(s'_i)
\end{align}
%TODO:continue
