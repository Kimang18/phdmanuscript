...
Fundamentally, there are at least three types of bandit problem depending on the nature of payoff: stochastic, adversarial, and Markovian.
...
There are two possible settings depending on whether the random random obtained by the decision maker is discounted over time or not.
...
In Rested Markovian bandit, each arm is modeled by a finite Markovian reward process. At each time step, the decision maker observes the current state of each arm and activates one arm.
...
Two divisions are made for restless bandit: (1) \textbf{partially observable} setup and (2) \textbf{fully observable} setup.

In partially observable restless bandit, each arm is a finite Markovian reward process.
...
The decision maker observes only the current state of activated arm.
That is why the term ``partially observable'' is used.

In fully observable restless bandit, each arm is a finite MDP with binary action space.
...
Last but not least, an even more general model of fully observable restless bandit is the case where each arm is a finite MDP with multiple actions.
The decision maker has to decide which action to be executed on each arm without violating the resource constraint.
...
We consider only \textbf{rested} and \textbf{fully observable restless} Markovian bandits.
For fully observable restless bandit, we will simply say restless bandit.
...
\subsubsection{Notations and problem formulation}
...
\subsubsection{Gittins index policy}
...
\subsubsection{Gittins index computation}
...
\subsection{Restless Markovian bandit}
...
\subsubsection{Whittle index policy}
...
\subsubsection{Whittle index computation}
