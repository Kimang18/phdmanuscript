...
\begin{equation}
    ...
    W_{M,{h:H}}^\pi(\vs)  :=  r(s_a) {+}\sum_{\vs'\in\gX}p(\vs'\mid \vs,a)W_{M,{h+1:H}}^\pi(\vs'),
\end{equation}
with $W_{M,{H:H}}^\pi(\vs):=r(s_a)$. % and where $r^\pi$ and $P^\pi$ are reward vector and state transition matrix when following policy $\pi$.
