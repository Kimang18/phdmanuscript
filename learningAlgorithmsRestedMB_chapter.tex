...
In Section~\ref{ch:rested:sec:contribute}, we summarize our results in learning with rested Markovian bandit with discount.
...
\begin{thm}[Optimistic arms do not form optimistic bandit]
    \label{thm:no_OFU}
...
\end{align*}
This means that restricting to index policies is not a restriction for the optimism. What Theorem~\ref{thm:no_OFU} shows is that an optimistic index policy can be defined only after the optimistic MDP $M'\in\sM$ is computed and computing optimistic policy and $M'$ simultaneously depends on the confidence bonuses of all arms.
