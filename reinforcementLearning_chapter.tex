...
Model-based algorithms input the estimates of $r$ and $p$ to a planning method described in the previous chapter such as backward induction, policy or value iterations and follows the policy output by the planning method.
...
\subsection{Infinite horizon average reward model}
...
As mentioned in Section~\ref{sec:intro_learning}, we consider model-based algorithms with \textbf{episodic policy update}.
...
In Bayesian approach, we say the unknown MDP $M$ is drawn from a probability distribution (prior or posterior) $\phi$ to mean that $r$ is drawn from a dedicated probability distribution (prior or posterior) and $p$ is drawn from another dedicated probability distribution (prior or posterior).
...
A common process between the two approaches is that they compute an optimal policy of an ``imagined MDP'' that they believe to be the unknown MDP $M=\langle\gS,\gA,r,p\rangle$.
...
\begin{algorithm}[ht]
    ...
    %\Init{$t^1=1, o_{t^1}=\{s_1\}$}
    \BlankLine
    \For{episodes $k=1,2,\dots$}{
        %Based on observation $o_{t^k}$, compute the imagined MDP $M^k$ (using posterior sampling or optimism). \;\label{line:new_mdp}
        Compute the imagined MDP $M^k=\langle\gS,\gA,r^k,p^k\rangle$ \;\label{line:new_mdp}
        Compute an optimal policy $\pi^k$ of $M^k$ \;\label{line:new_policy}
        \While{episode $k$ continues}{
...
We summarize several existing results about minimizing the regret in Table~\ref{ch:rl:tab:finite} for finite horizon setting and Table~\ref{ch:rl:tab:infinite} for infinite horizon average reward model.
\begin{table}[ht]
    \centering
\begin{tabular}{|l|l|}
\hline
Algorithm & Regret \\ \hline
PSRL \cite{osband2013more}  & $\tilde{\landauO}(H\abs{\gS}\sqrt{\abs{\gA}T})$ \\ 
UCBVI-BF \cite{azar2017minimax}  & $\tilde{\landauO}(\sqrt{H\abs{\gS}\abs{\gA}T})$ \\ 
EULER \cite{zanette2019tighter}        & $\tilde{\landauO}(\sqrt{H\abs{\gS}\abs{\gA}T})$ \\ \hline
Lower bound & $\landauOmega(\sqrt{H\abs{\gS}\abs{\gA}T})$ \cite{jaksch2010near} \\ \hline
\end{tabular}
\caption{Short regret summary for model-based RL algorithms in finite horizon setting with $\abs{\gS}$ states, $\abs{\gA}$ actions, and $T=KH$ steps.
...
REGAL \cite{bartlett2012regal}      & $\tilde{\landauO}(H\abs{\gS}\sqrt{\abs{\gA}T})$  & weakly comm. & Intractable   \\ 
UCRL2 \cite{jaksch2010near}       & $\tilde{\landauO}(D\abs{\gS}\sqrt{\abs{\gA}T})$ & diameter is finite        & EVI \\ 
TSDE \cite{ouyang2017learning}        & $\tilde{\landauO}(H\abs{\gS}\sqrt{\abs{\gA}T})$ & weakly comm. & VI       \\ 
OSP \cite{ortner2020regret}         & $\tilde{\landauO}(\sqrt{t_{mix}\abs{\gS}\abs{\gA}T})$ & ergodic           & Intractable   \\ 
SCAL \cite{fruit2018efficient}        & $\tilde{\landauO}(H\abs{\gS}\sqrt{\abs{\gA}T})$ & weakly comm. & modified EVI           \\
%KL-UCRL     & $\tilde{O}$(\sqrt{S\sum_{x,a}\sV^*_{x,a}T}) & ergodic                     & modified EVI                  \\ \hline
UCRL2B \cite{fruit2020improved}      & $\tilde{\landauO}(\abs{\gS}\sqrt{D\abs{\gA}T})$ & diameter is finite      & EVI                           \\
EBF \cite{zhang2019regret}         & $\tilde{\landauO}(\sqrt{H\abs{\gS}\abs{\gA}T})$ & weakly comm.     & Intractable   \\ \hline
Lower bound & $\landauOmega(\sqrt{D\abs{\gS}\abs{\gA}T})$ \cite{jaksch2010near} &                              &                               \\ \hline
\end{tabular}
\caption{Short regret summary for model-based RL algorithms in infinite horizon average reward model with $\abs{\gS}$ states, $\abs{\gA}$ actions, and $T$ steps.
$D$ is the diameter of the MDP, $H\ge sp(\vh^*)$ is upper bound on the span of the optimal bias, $t_{mix}$ is the mixing time of the MDP.
...
\subsection{Upper bound on regret : general idea}
...
\subsection{Sketch of proof on regret bound of three algorithms}
...
\end{align*}
where $\beta_r^k(s,a)=\displaystyle\sqrt{\frac{c_r\ln(\abs{\gS}\abs{\gA}t^k)}{2\max\Bigl(1,n^k(s,a)\Bigr)}}$ and $\beta_p^k(s,a)=\displaystyle\sqrt{\frac{c_p\abs{\gS}\ln(\abs{\gA}t^k)}{\max\Bigl(1,n^k(s,a)\Bigr)}}$ are the confidence bonuses, and $c_r$ and $c_p$ are constants to be chosen accordingly before the learning.
...
Differently from UCRL2 that chooses only one copy of expected reward, UCBVI chooses $H$ copies in the following manner: when planning for episode $k$,
\begin{itemize}
...
\end{align*}
where $\beta_r^k(s,a,h)=\displaystyle h\sqrt{\frac{c_r\ln(\abs{\gS}\abs{\gA}t^k)}{2\max\Bigl(1,n^k(s,a)\Bigr)}}$ is the confidence bonus, and $c_r$ is a constant to be chosen accordingly before the learning.
...
\subsubsection{Posterior sampling for reinforcement learning (PSRL) \texorpdfstring{\cite{osband2013more}}{[ORV13]}.}
...
Broadly speaking, we bound the Bayesian regret by bounding the regret with terms that are deterministic and the expected value of those terms are themselves.
