...
So, we say the MDP $M$ is unknown to the learner.
The fundamental goal of the learner is to identify an optimal policy $\pi^*$ by interacting with the unknown MDP.

\subsection{Overall description of learning}
...
This mechanism is used in \textbf{offline learning paradigm} in which the learner keeps collecting samples until a budget resource fixed by the setting is exhausted (it can be a time resource, sample resource or approximation error). Then, the learner follows a policy and no more policy update is made thereafter.
...
\begin{enumerate}
    \item \textbf{model-free}: the learner tries to infer the value functions
    \item \textbf{model-based}: the learner tries to infer the unknown parameters $r$ and $p$ of the MDP $M$.
\end{enumerate}
...
\subsection{Learning framework in this thesis}

This thesis focuses on \emph{model-based algorithms in online learning with navigating model}.
...
To measure the performance of the learner, we use the notion of \emph{regret} that compares the cumulative rewards of an optimal policy $\pi^*$ and the learner. 
...
\begin{defn}
    ...
    \end{equation}
\end{defn}
Since $H$ is bounded, we derive from Definition~\ref{defn:rg_finite} that a ``good'' learner has an expected regret sublinear in the number of episodes $K$, $\ex{\Reg(\gL, M, K)}=\landauo(K)$ when $K\to+\infty$.
...
So, the regret is linear in the total number of time steps $T$.
