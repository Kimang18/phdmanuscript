...
\section{A brief summary of existing learning paradigms}
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Generative model}: the learner can restart at desire and in any state of $M$
    \item \textbf{Navigating model}: the learner cannot control the restart.
\end{enumerate}
With the generative model, the learner chooses any state-action pair $(s,a)$ and the MDP $M$ incurs a random reward $u$ whose expected value is $r(s,a)$ and the next state $s'\in\gS$ with probability $p(s'\mid s,a)$.
...
For both paradigms described above, there are two types of learning
\begin{itemize}
...
\end{itemize}
In frequentist school, the learning mechanism is decided before the learning begins and the observations collected during the learning are used to controlled \textbf{the probability that the mechanism achieves its goal}.
In Bayesian school, the observations collected during the learning are used to \textbf{update the posterior distribution of the random variables}.

%We will discuss about two families of model-based algorithms in the below sections.

\section{Learning paradigm studied in this thesis}
\label{sec:intro_learning}

We focus on \textbf{model-based} algorithms in online learning with \textbf{navigating model and episodic policy update}.
...
If $H\in\real$ is an upper bound on $sp(\vh^*)$, then the minimax bound is also expressed as $\landauOmega(\sqrt{HSAT})$.
...
For both approaches, Line~\ref{line:new_policy} of Algorithm~\ref{algo:generics} can be broken down into two steps
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
We will say that $M^k$ is the imagined version of the unknown $M$ in episode $k$.
...
\begin{algorithm}[ht]
    ...
    \Init{}{Observe initial state $s_1$}
    \BlankLine
    \For{episodes $k=1,2,\dots$}{
        ...
        Compute a new policy $\pi^k$ (using optimism or posterior sampling) \;\label{line:new_policy}
...
That is, at Line~\ref{line:new_policy} of Algorithm~\ref{algo:generics}, UCRL2 computes a new policy $\pi^k$ such that
\begin{equation}
...
Differently from UCRL2 that chooses only one copy of expected reward, UCBVI executes Line~\ref{line:new_policy} of Algorithm~\ref{algo:generics} by choosing $H$ copies in the following manner:
\begin{itemize}
...
Then, Line~\ref{line:new_policy} of Algorithm~\ref{algo:generics} is performed by sampling a MDP $M^k$ from $\phi\bigl(\cdot\mid o_{t^k}\bigr)$ and computing an optimal policy $\pi^k$ in $M^k$.
