...
In this chapter, we discuss the existence of the Whittle index in restless Markovian arm with average reward criterion and provide a few main contributions to characterize such an existence.
...
We recall from Chapter~\ref{ch:mb} that the quantity $\lambda$ is a penalty for taking action ``activate''.
...
The most problematic choice is the notion of increasingness that applies to both discounted and average reward criteria: Interpretation $(\exists)$ is more permissive: For instance, consider an arm with two states, and assume that the optimal policy is $\{1,2\}$ for $\lambda<0$, and either $\{1\}$ or $\emptyset$ for $\lambda>0$.
...
\begin{figure}[ht]
    ...
    \caption{Example in which the classical definition of indexability is ambiguous: All transitions are deterministic, and all labels on transitions indicate rewards. Solid black arrows correspond to the action ``activate'', and dashed red arrows to the action ``rest''.
}
...
\begin{defn}[Bellman optimal policy]
    ...
    \begin{equation}
        ...
        \sum_{j=1}^nP^{\pi_i}_{ij}g^*_j =g^*_i \text{ and } \pi_i\in\argmax_{a\in\{0,1\}} \Big(r^{a}_i - \lambda a_i + \sum_{j=1}^n P^{a}_{ij}h^*_j\Big).
    \end{equation}
...
The Bellman optimality is very natural, and we say that it is new because, to the best of our knowledge, the definition is never introduced in the literature of MDP.
Note that the distinction between gain optimality and Bellman optimality disappears in infinite horizon discounted problems or in ergodic MDPs with average reward criterion.
...
As we show next, Definition~\ref{ch:idx:defn:idx} guarantees that Whittle index is uniquely defined when it exists.
...
By doing so, we already solve the ambiguity in the indexability of discounted restless bandits.
...
However, we must recognize that this new definition still does not cover all the problem of indexability in restless bandits with average reward criterion.
   ...
   \begin{tabular}{ccc}
       ...
       \begin{minipage}{.35\linewidth}
           ...
           \path[->]
               ...
               (B) edge[color=black, loop above] node{$-1000-\lambda$} (B)
               (C) edge[color=red, dashed,loop below] node{$-1000$} (C)
       ...
       (a) & (b)
    \end{tabular}
    \caption{Examples that are non-indexable when using Bellman optimality but indexable when using a stronger notion of optimality: All transitions are deterministic, and all labels on transitions indicate rewards. Solid black arrows correspond to the action ``activate'', and dashed red arrows to the action ``rest''.
}
    \label{fig:ambiguous_example2}
\end{figure}

For example (a), the Bellman optimal and gain optimal policies are identical: policy $\{1,2\}$ for $\lambda<0$, and $\{1\}$ and $\emptyset$ for $\lambda>0$.
...
This example is a unichain $2$-state MDP, where state $1$ is the transient state and allows two possible actions, and state $2$ is the recurrent state and allows only one possible action.
...
\subsection{Conditions for the uniqueness of the Bellman optimal policy}
...
We will see in the next chapter that this direction will require the uniqueness of the Bellman optimal policy.
...
\subsubsection{Uniqueness of the Bellman optimal policy}
...
So, the following lemma provides a condition that can be used to verify if a given Bellman optimal and unichain policy is the unique Bellman optimal policy.
\begin{lem}[Condition for the uniqueness of Bellman optimal policy]
...
%While this lemma provides a characterization of uniqueness for Bellman optimal policy in average reward criterion, the Bellman optimality coincides with the classical optimality in infinite-horizon discounted MDPs.
%So, the lemma can be equally used to characterize uniqueness of optimal policy in discounted MDPs.
...
Along with this new definition of indexability, we gave the corresponding definition of Whittle index.
We introduced a new notion of Bellman optimality for MDPs with average reward criterion. We derived a technical lemma to check if a given Bellman optimal policy was unique.
This uniqueness property will play a crucial role in computing Whittle index.
