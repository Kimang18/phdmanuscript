...
By \cite[Theorem~9.1.7]{puterman2014markov}, a gain optimal policy can be identified by any solution $(\vg^*, \vh^*)$ of \eqref{eq:gain_opt} and \eqref{eq:bias_opt} (recall that $\vg^*$ is the optimal gain and uniquely defined).
...
Yet, $\pi^1\notin\argmax_{\pi'}\Bigl(\vr^{\pi'}+\mP^{\pi'}\vh^*\Bigr)$ for any $\vh^*$ that satisfies \eqref{eq:bias_opt} while $\pi^2$ does.
...
Lemma~\ref{ch:mbp:lem:bellman_charac} shows that a policy is Bellman optimal if and only if it satisfies the optimality equation \eqref{eq:bias_opt} on all states for some $\vh^*$.
...
\begin{proof}
    ...
    By \cite[Theorem~8.2.6]{puterman2014markov}, the gain of policy $\pi$ can be expressed by $g^\pi_i=\sum_{j\in[n]}\bar{P}^\pi_{ij}r^{\pi}_j$ for any $i\in\gS$.
    ...
    The latter rewrites as $\sum_{i\in[n]}\bar{P}^\pi_{ki}\Big(r^{\pi}_i-g^*_i\Big) =0$ for all state $k$. For some $\vh^*$ that satisfies \eqref{eq:bias_opt} with $\vg^*$, we have: for all state $k$
    \begin{align*}
    ...
    This shows that $B^{\pi_i}_i(\vg^*,\vh^*) =0$ for all $i\in\Phi^\pi$.
\end{proof}
...
In such MDPs, the distinction between gain and Bellman optimal disappears.
...
As mentioned in Section~\ref{ch:idx:sec:classic_idx}, we will use the interpretation ``all optimal policies'' to define indexability.
...
That is, in \figurename~\ref{fig:gain_vs_bellman}, the bias of policy $\pi^1$ is $h^{\pi^1}_1=c^{\pi^1}-0.5$ and $h^{\pi^1}_2=c^{\pi^1}$ and the bias of policy $\pi^2$ is $\vh^{\pi^2}_1=c^{\pi^2}\vone$ where $c^{\pi^1}$ and $c^{\pi^2}$ are any real numbers.
Recall that $\vh^{\pi^1}$ is the solution of the evaluation equation \eqref{eq:bias_eval} for $\pi^1$ and $\vh^{\pi^2}$ for $\pi^2$.
...
\begin{proof}
    ...
    Since $\pi$ and $\theta$ are Bellman optimal, $\vh^\pi$ and $\vh^\theta$ satisfy the optimality equation \eqref{eq:bias_opt} together with $\vg^*$.
    ...
    \end{align*}
    with equality for all state $i\in\Phi^\theta$.
    ...
    Furthermore, by definition advantage function, $B_i^{\pi_i}(\vg^*, \vh^\pi)=B_i^{\pi_i}(\vg^*, \vh^\theta)=0$ and $B_i^{\theta_i}(\vg^*, \vh^\theta)=B_i^{\theta_i}(\vg^*, \vh^\pi)=0$ for all state $i$.
\end{proof}

\subsubsection{Unicity of Bellman optimal policy.}
\label{ch:idx:sssec:unicity}

By Definition~\ref{ch:mbp:defn:bellman}, checking if the MDP has a single Bellman optimal policy is not trivial due to the multiplicity of the solution $\vh^*$ of the optimality equation \eqref{eq:bias_opt} given $\vg^*$.
...
\begin{proof}
    ...
    Otherwise, $\theta_i=\pi_i$ for any $i\in\Phi^\theta$.
...
\end{proof}
Lemma~\ref{ch:mbp:lem:unicity_BO} implies that a Bellman optimal and unichain policy $\pi$ is the unique one if and only if $B_i^a(\vg^*, \vh^\pi)<0$ for all action $a\neq\pi_i$ and all state $i$.
...
Also, it will play in an important in characterizing the indexability of restless Markovian arm.
...
We now continue the discussion about indexability based on $\lambda$-penalized MDP and reintroduce $\lambda$ in the notation.
...
So, the definition implies that the problem is indexable.
...
The proposition below shows that Definition~\ref{ch:idx:defn:idx} implies that Whittle index is well defined and proposes a characterization of any indexable arm, that we will later use to derive our algorithm. 
\begin{lem}
...
\begin{proof}
    ...
    $(ii)\Rightarrow(i)$ -- Assume $(ii)$ and for $k\in[n]$, let $\sigma^k$ be the state with the $k$th smallest index (where ties are broken arbitrarily).
