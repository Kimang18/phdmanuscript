...
Our algorithm works in discounted and undiscounted cases and can compute the Gittins index.
This algorithm will be built on three tools: (1) a careful characterization of Whittle index that allows one to recursively compute the $k$th smallest index from the $(k-1)$th smallest, and to test the indexability, (2) the use of the Sherman-Morrison formula to make this recursive computation efficient, and (3) a sporadic use of the fastest matrix inversion and multiplication methods to obtain a subcubic complexity.
...
This work is part of our article submitted to Mathematical Methods of Operations Research (MMOR) journal (\cite{gast2022computing}).
...
Moreover, the algorithm can detect on the fly if a computed index violates the indexability condition, adding an extra $(1/3)n^3+O(n^2)$ arithmetic operations. This test is optional: the complexity of our algorithm is $n^3+o(n^3)$ when testing the indexability and $(2/3)n^3 +o(n^3)$ without the test. For discounted problems, our algorithm works for any finite-state arm regardless of its structure. For undiscounted problems, our algorithm takes as input any arm and outputs three possibilities: the arm is (1) indexable, (2) non-indexable, or (3) multichain. % These three cases are not mutually exclusive.
...
For undiscounted restless bandits $\gamma=1$, the author of \cite{nino2020fast} only provides a brief description of how the fast-pivoting algorithm developed for discounted case can be adapted, although no explicit algorithm is given in that paper.
