...
Since the state space $\gS$, the states of MDP are either \emph{transient} or \emph{recurrent}.
A state is \emph{transient} if it is never visited in steady regime.
For more formal definition, one can refer to \cite[Appendix~A]{puterman2014markov} or \cite{levin2017markov}.
...
\begin{defn}[Classification of MDPs]
    ...
    \begin{enumerate}
        ...
        \item \textbf{unichain} if the Markov chain induced by any stationary policy is \emph{unichain}\footnote{we also say that the corresponding policy is unichain}, \ie, it has a single recurrent class plus a --possibly empty-- set of transient states;
        ...
        \item \textbf{weakly communicating} if the state space can be partitioned into two subsets $\gS^C$ and $\gS^T$ (with $\gS^T$ possibly empty), such that for every pair of states $(s,s')\in\gS^C$, there exists a stationary policy under which $s'$ is accessible from $s$ in finite time with non-zero probability, and all states in $\gS^T$ are transient under all stationary policies.
    \end{enumerate}
\end{defn}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        ...
        \draw[dashed] (0,2.4) -- (7.,2.4);%  (-2,1) -- (6.5,1);
        \node at (7,3) {multichain};
        \node at (7,2) {unichain};
        % \node[text width=2cm, align=center] at (-1.5,0.5) {discounted problems};
        \draw[line width=2pt, fill=blue,opacity=0.3] (0,2.4) rectangle (6,4);
        \node at (3,3.5) {};
        \draw[line width=2pt, fill=green,opacity=0.3] (0,1) rectangle (6,3.6);
        \node[text width=4cm,align=center] at (3,3.3) {Weakly communicating};
        \draw[line width=2pt, fill=red,opacity=0.3] (0.5,1.2) rectangle (5.5,3);
        \node[text width=2cm,align=center] at (2.7,2.7) {Communicating};
        \draw[line width=2pt, fill=white,opacity=0.3] (1,1.6) rectangle (5,2.2);
...
Verifying if a MDP is unichain is NP-hard\cite{tsitsiklis2007np}. However, verifying if a MDP is weakly communicating is feasible in $\landauO(\abs{\gS}^2)$ using Tarjan's strongly connected component algorithm.
That is, we construct a transition matrix ...
