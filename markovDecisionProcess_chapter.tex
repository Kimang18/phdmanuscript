...
The structure of solution space for $\vh$ given $\vg^*$ is well studied in \cite{schweitzer1978functional}.
...
To understand the our new notion of optimality, we first need to characterize the gain optimal policies.
...
\end{equation}
If $\pi$ is unichain, then $\vh$ is determined up to a constant vector and $B^a_s(\vh)$ is then uniquely defined for any $s\in\gS$, $a\in\gA_s$ and $\vh\in\R^{\abs{\gS}}$ that satisfies \eqref{eq:bias_eval}.
...
With this characterization, in ergodic MDPs, a policy is gain optimal if and only it acts optimally in all states.
However, for MDPs that admit transient states, a policy can be gain optimal without acting optimally in all states, \ie, it just acts optimally in its recurrent states.
...
\begin{figure}[ht]
    ...
    \begin{tabular}{cc}
        ...
        \begin{minipage}{.7\linewidth}
            ...
            So, we want to characterize $\pi^2$.
        \end{minipage}
    \end{tabular}
