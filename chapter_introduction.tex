...
In Chapter~\ref{ch:index_computation}, we present our new algorithm to test indexability presented in the previous chapter and compute the Whittle index.
...
In Chapter~\ref{ch:learning_rested}, we consider an episodic RL problem in which the unknown environment is a discounted rested Markovian bandit having $n$ arms and $S$ states per arm and the episode length is geometrically distributed.
...
height=1cm,text centered, draw=black, fill=blue!5, drop shadow},
...
\node (mdp) [rect, label={[font=\small\sffamily,name=part1]above:{Part I: Background}}] {Chapter 2: Markov Decision Process};
\node (rl) [rect, below left = 0.75cm and -2.5cm of mdp] {Chapter 3: Reinforcement Learning};
\node (mb) [rect, below right = 0.75cm and -2.1cm of mdp] {Chapter 4: Markovian Bandits};

\node (idx) [rect, below = 1.75cm of mb, label={[font=\small\sffamily,name=part2]above left:{Part II: Indexability}}] {Chapter 5: Indexability};
\node (widx) [rect, below = 0.75cm of idx] {Chapter 6: Index Computation};

\node (rested) [rect, below = 1.75cm of rl, label={[font=\small\sffamily,name=part3]above left:{Part III: Learning}}] {Chapter 7: Learning Rested Bandit};
